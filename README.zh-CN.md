...
| ![攻防演练-进行中](docs/screenshots/drill-running.png) | ![光电视窗-锁定](docs/screenshots/drill-eo-lock.png) |
| **攻防演练(进行中)**——敌机突入识别圈,实时告警滚动,AI 自动守候择优处置并给出理由 | **光电视窗**——白光/热成像双光谱 HUD,自动寻的,点击**锁定**后视轴跟随,联动范围内反制装备处置 |
| ![攻防演练-布防](docs/screenshots/drill-placed.png) | ![天地图引擎](docs/screenshots/monitor-tdt.png) |
| **攻防演练(布防)**——反制装备拖入地图,按类型默认扫描/作用范围画圈 | **实时监控·天地图**——国家官方底图引擎,围栏与设备覆盖物不变 |

## 功能总览

| 模块 | 说明 |
|------|------|
| 实时监控大屏 | 多引擎底图,无人机图标实时位置与航向、航迹线、电子围栏渲染(圆/线/面);**实时告警移至顶栏时间旁**(铃铛 + 未读角标,下拉面板,WebSocket 实时推送),右侧 **无人机(在线·离线双 TAB)/ 物联传感器 / 视频监控** 面板均可上下收缩,整栏亦可一键收起(地图占满);地图支持 **一键全屏**(Esc 退出);无人机均支持 **轨迹回放**(时间轴拖动/倍速;在飞=实时入库,离线=近 3 天历史);地图展示全部设备 ICON(**优先渲染「设备管理」配置的自定义图标**),点击查看基础信息 + 近 60 分钟历史曲线;右上角 **地图工具箱**:比例尺 / 底图工具条 / 方向盘平移 / 自绘鹰眼缩略图 / 3D 罗盘 / 测距 / 面积测算(跨引擎一致,开关状态本地记忆) |
| **攻防演练** | 攻防演练页:将 **六类反制装备**(警戒雷达/无线电探测/光电跟踪/无线电压制/激光处置/网捕无人机)**拖入地图布防**(各按类型默认扫描/作用范围画圈)或一键布防;投放 1–8 架敌机(侦察巡飞机/快速穿越机)突入识别圈:**探测 → 识别跟踪 → 入侵告警**;可逐机手动处置(压制驱离 / 激光击落·需已跟踪 / 网捕捕获,带冷却),或开启 **AI 自动守候** 由大模型择优处置并给出理由;支持 1×/2×/4× 倍速、增派敌机、评分与结束汇总(驱离/逃离/平均响应),演练记录入库 |
| **光电视窗** | 白光/热成像双光谱视窗,1×/2×/4× 变倍 HUD:方位刻度尺、视轴十字、按距离缩放的目标框(状态着色);SCAN 自动缓动寻的,**点击目标框即选中并锁定**——视轴平滑跟随、四角括号脉动、状态转 TRACKING;范围内反制装备处置按钮随距出现(激光需已跟踪) |
...
| **设备接入** | 无人机/机巢/摄像头/气象站/ADS-B/网关/传感器 七类物联网设备 **+ 六类反制设备**(警戒雷达/无线电探测/光电跟踪/无线电压制/激光处置/网捕无人机,各带类型默认扫描范围,表单可改),**支持自定义地图图标**(上传 PNG/SVG);虚拟设备由内置模拟器供数,真实设备经 **Netty 网关** 三端口接入;上报数据入库留存(每设备保留最近 2000 条)并经 WebSocket 实时推送;飞行遥测每 4s 入库 |
